...
int main()
{
	...
	if(strcmp(cfilename,"logo.jpg")!=0)
	{
		free(ucpbuf);
                ucpbuf=NULL;
		printf("<html><head>\r\n");
...
*/
	...
	else if(ret_pid==0)
	{
		...
		for(i=0;i<NOFILE;i++)
			close(i);
		//system("cp /tmp/logo.jpg /mnt/ -rf> /dev/null 2>&1");
		//system("cp /tmp/logo.jpg /usr/www/images/ -rf> /dev/null 2>&1");
		//system("cp /tmp/logo.jpg /usr/www/images/logo_5.jpg -rf> /dev/null 2>&1");
		//system("chmod a+wrx /usr/www/images/*.jpg> /dev/null 2>&1");
		system("cp /tmp/logo.jpg /mnt/ -rf> /dev/null 2>&1");
		system("cp /tmp/logo.jpg /mnt/logo_5.jpg -rf> /dev/null 2>&1");
...
//		reboot(RB_AUTOBOOT);	
	...
	{
		printf("\r\n");
		 fp=fopen("/usr/www/Waitpic2_CH.html","r");
                while(!feof(fp))
                {
                        memset(waitbuf,0,1024);
                        fgets(waitbuf,1024,fp);
